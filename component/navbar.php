...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const currentPath = window.location.pathname.split("/").pop(); // ambil nama file aktif

  navLinks.forEach(link => {
    if (link.getAttribute("href") === currentPath) {
      link.classList.add("active", "text-success"); // highlight menu aktif
    } else {
...
.nav-link.active {
  ...
  font-weight: bold;
  transform: scale(1.1); /* efek animasi membesar */
}
</style>